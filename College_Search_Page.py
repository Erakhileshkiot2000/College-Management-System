...
class Window_1():
    def __init__(self, master):
        ...
        search_frame = LabelFrame(self.master, width=1000, height=100, font=(
            'arial', 30, 'bold'), relief='ridge', bd=15, bg='Green')
        search_frame.grid(row=1, column=0, padx=200, pady=200)

        lbl_enterrollnumber = Label(
            search_frame, text='Enter Roll Number', font=('arial', 25, 'bold'), bg='Green')
        lbl_enterrollnumber.grid(row=0, column=0, padx=100, pady=10)

        txtsearch_entry = Entry(search_frame, font=(
            'arial', 25, 'bold'), textvariable=self.roll)
        ...
        def Search():
            if(len(self.roll.get()) != 0):
                ...
                tkinter.messagebox.askokcancel(
                    'Attention', 'Please enter valid Roll No.')
                return
        ...
        buttone_new = Button(search_frame, text='CREATE NEW',
                             width=15, font=('arial', 15, 'bold'), command=new)
