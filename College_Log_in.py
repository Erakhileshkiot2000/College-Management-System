...
class Screen_Window_1:
    def __init__(self, master):
        ...
        # x = StringVar()  Holds a string; default value is " "
        self.u_name = StringVar()
        self.u_password = StringVar()

        self.Lbl_Title_Login = Label(self.Frame, text='Login Menu', font=(
            'arial', 55, 'bold'), bg='Green', fg='Black')
        self.Lbl_Title_Login.grid(row=0, column=0, columnspan=3, pady=40)

        self.Log_Frame_1 = LabelFrame(self.Frame, width=1350, height=600, relief='ridge', bg='Green', bd=20,
                                      font=('arial', 20, 'bold'))
        self.Log_Frame_1.grid(row=1, column=0)
        self.Log_Frame_2 = LabelFrame(self.Frame, width=1000, height=600, relief='ridge', bg='Green', bd=20,
                                      font=('arial', 20, 'bold'))
        self.Log_Frame_2.grid(row=2, column=0)

        # ===================================================LABEL and ENTRIES=======================================================================
        self.lbl_uname = Label(self.Log_Frame_1, text='Username', font=(
            'arial', 20, 'bold'), bg='Green', bd=20)
        self.lbl_uname.grid(row=0, column=0)
        self.txt_uname = Entry(self.Log_Frame_1, font=(
            'arial', 20, 'bold'), textvariable=self.u_name)
        self.txt_uname.grid(row=0, column=1, padx=50)

        self.lbl_pass = Label(self.Log_Frame_1, text='Password', font=(
            'arial', 20, 'bold'), bg='Green', bd=20)
        self.lbl_pass.grid(row=1, column=0)
        self.txt_pass = Entry(self.Log_Frame_1, font=(
            'arial', 20, 'bold'), show='*', textvariable=self.u_password)
        ...
        self.exit_button = Button(self.Log_Frame_2, text='Exit', width=10, font=(
    ...
    def Exit(self):
        self.Exit = tkinter.messagebox.askokcancel(
...
if __name__ == '__main__':
    root = Tk()
    applicaton = Screen_Window_1(root)
    root.mainloop()
